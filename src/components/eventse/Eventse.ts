  ...
  {preview : "K",id: 'Eldarion', start: 120, end: 220, 'age': "FO", startLabel: "", endLabel: "",color:"#69b3a2"}]
   ...
   getTooltipContent (d:any) {
    ...
    updateView(start:any,end:any){
      ...
      .domain([d3.min(this.viewData, (d:any) => d.start), d3.max(this.viewData, (d:any) => d.end)])
      .range([0, this.width - this.margin.left - this.margin.right]);

      this.newXAxis = d3.axisBottom(this.newx)

      this.contex.selectAll("g.x.axis")
      .call(this.newXAxis);
    ...
    this.groupsView.attr("transform", (d:any,i:any) => `translate(0,${this.y(d.age)})`);

    this.groupsView
    .each((this.getRectView))
    .attr("pointer-events", "all")
    .on("click",(d:any) => {
      this.tooltip2.style("opacity", 1)
      ...
      this.div.transition()		
                .duration(200)		
                .style("opacity", .9);		
            this.div.html(this.getTooltipContent(d))	
                .style("left", (d3.event.pageX) + "px")		
                .style("top", (d3.event.pageY - 28) + "px");
      
    })
    .on("mouseover", (d:any,i:any,n:any) =>{
    d3.select(n[i]).select("rect").attr("fill", this.LightenDarkenColor(d.color,-20));
    ...
    .on("mouseleave", (d:any,i:any,n:any)=> {
     ...
     brushed() {
      ...
      if (d3.event.selection) {
        this.focus.property("value", d3.event.selection.map(this.x.invert, this.x));
        this.height = 1;
...
this.y= d3.scaleBand()
.domain(this.data.map(function(d:any) { return d.age; }))
.range([0,this.height ])
.padding(0.2);
...
createAxises(){
   ...
   this.axisLeft2 = d3.axisLeft(this.y2)
    .tickPadding(2);
...
mounted()
{
  ...
  this.focus = this.svg.append("g")
    ...
    */  
    ...
    //this.groups.attr("transform", (d:any,i:any)=>`translate(0 ${this.y(i)})`);
    this.groups.attr("transform", (d:any,i:any) => `translate(0,${this.y2(d.age)})`);
 
    this.tooltip = d3.select(document.createElement("div")).call(this.createTooltip);
    this.tooltip2 = d3.select("#Events").append("div")	
    .attr("class", "tooltip")				
    .style("opacity", 0);

     this.div = d3.select("#Events").append("div")	
    .attr("class", "tooltip")				
    .style("opacity", 0);
...
/*
    this.focus.on("mousemove", (d:any)=> {
      ...
      this.contex = this.svg.append("g")
    .attr("class", "context")
    .attr("transform", "translate(" + 60 + "," + this.margin.top  + ")");


    this.contex.append("g")
    .attr("class", "x axis")
    ...
    this.gView = this.focus.append("g")
